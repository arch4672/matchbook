...
class ReportingTest(unittest.TestCase):
    ...
    @mock.patch(
        "matchbook.endpoints.reporting.Reporting.request", return_value=mock.Mock()
    )
    def test_get_old_transactions_report(self, mock_request, mock_process_response):
        self.reporting.get_old_transactions_report(
            offset=0, per_page=500, categories=TransactionCategories.Exchange
        )
    ...
    def test_get_new_transactions_report(self, mock_request, mock_process_response):
        self.reporting.get_new_transactions_report(
            transaction_type=TransactionTypes.All, offset=0, per_page=500
        )
