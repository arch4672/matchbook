...
class MarketDataTest(unittest.TestCase):
    ...
    @mock.patch(
        "matchbook.endpoints.marketdata.MarketData.request", return_value=mock.Mock()
    )
    def test_get_events(self, mock_request, mock_process_response):
        self.market_data.get_events(
            sport_ids="9",
    ...
    def test_get_markets(self, mock_request, mock_process_response):
        self.market_data.get_markets(
            event_id=123, offset=0, per_page=500, price_depth=3
        )
    ...
    def test_get_markets_single(self, mock_request, mock_process_response):
        self.market_data.get_markets(
            event_id=123, market_id=345, offset=0, per_page=500, price_depth=3
        )

        mock_request.assert_called_once_with(
            "GET",
            self.client.urn_edge,
            "events/123/markets/345",
            session=None,
            params={
                "event-id": 123,
                "market-id": 345,
    ...
    def test_get_runners(self, mock_request, mock_process_response):
        ...
        mock_request.assert_called_once_with(
            "GET",
            self.client.urn_edge,
            "events/123/markets/345/runners",
            session=None,
            target="runners",
            params={
                "event-id": 123,
    ...
    def test_get_runners_single(self, mock_request, mock_process_response):
        ...
        mock_request.assert_called_once_with(
            "GET",
            self.client.urn_edge,
            "events/123/markets/345/runners/567",
            session=None,
            params={
                "event_id": 123,
                "market-id": 345,
